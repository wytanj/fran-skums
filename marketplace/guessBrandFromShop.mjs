 ...
 * Used by tests; keep in sync with extensions/skums-shopee-shop-resolve/brandMatch.js
 */
...
export function needsShop(b) {
  return !b.shop_username || b.shop_resolve_status !== 'confirmed'
}

export function scoreBrandAgainstShop(brand, shopUsername, pageTitle) {
 ...
 */
export function guessBrandForShop(brands, opts = {}) {
  ...
  for (const b of list) {
    ...
    if (opts.prefer_unconfirmed !== false && needsShop(b) && s >= min) s += 2
    if (s < min) continue
...
export function usernameFromShopUrl(url) {
  try {
    const u = new URL(url)
