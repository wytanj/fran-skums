...
<template>
  <div class="flex flex-col items-center justify-center rounded-xl border border-dashed border-line px-6 py-14 text-center">
    <div class="mb-3 flex h-[68px] w-[68px] items-center justify-center rounded-full bg-surface-sunken text-2xl">
      <slot name="icon">
    ...
    <p v-if="description" class="mt-1.5 max-w-sm text-[13px] text-muted">{{ description }}</p>
    <div v-if="actionLabel" class="mt-5">
      <NuxtLink v-if="actionTo" :to="actionTo" class="btn-primary">
        {{ actionLabel }}
      </NuxtLink>
      <button v-else type="button" class="btn-primary" @click="emit('action')">
        {{ actionLabel }}
      </button>
    </div>
  </div>
