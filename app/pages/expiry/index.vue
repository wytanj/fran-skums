...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-ink">Expiry</h1>
        <p class="mt-1 text-sm text-muted">Track batch expiries, plan LIFO, and share transparency pages.</p>
      </div>
    </div>
    ...
    <div v-show="activeTab === 'overview'" class="space-y-6">
      ...
      <div v-if="summary" class="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div class="card p-4">
          <p class="text-xs text-muted uppercase tracking-wide">In Stock Items</p>
          <p class="mt-1 text-2xl font-bold text-ink">{{ summary.total_items }}</p>
          <p class="text-xs text-muted">{{ summary.total_quantity }} total units</p>
        </div>
        <div class="card p-4">
          <p class="text-xs text-muted uppercase tracking-wide">Expired</p>
          <p class="mt-1 text-2xl font-bold" :class="summary.expired > 0 ? 'text-danger' : 'text-ink'">{{ summary.expired }}</p>
          <p class="text-xs text-muted">need disposal or promo</p>
        </div>
        <div class="card p-4">
          <p class="text-xs text-muted uppercase tracking-wide">Expiring < 30d</p>
          <p class="mt-1 text-2xl font-bold" :class="summary.expiring_30d > 0 ? 'text-warning' : 'text-ink'">{{ summary.expiring_30d }}</p>
          <p class="text-xs text-muted">high priority</p>
        </div>
        <div class="card p-4">
          <p class="text-xs text-muted uppercase tracking-wide">Expiring < 90d</p>
          <p class="mt-1 text-2xl font-bold" :class="summary.expiring_90d > 0 ? 'text-warning' : 'text-ink'">{{ summary.expiring_90d }}</p>
          <p class="text-xs text-muted">plan promotions</p>
        </div>
      </div>
      ...
      <div v-if="summary && summary.unresolved > 0" class="rounded-lg border border-yellow-500/30 bg-yellow-500/5 p-4">
        <div class="flex items-start gap-3">
          <svg class="mt-0.5 h-5 w-5 shrink-0 text-warning" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
      ...
      <div class="card p-6">
        <h2 class="mb-4 text-lg font-semibold text-ink">Soonest Expiring</h2>
        <div v-if="lifoItems.length > 0" class="space-y-2">
          ...
          >
            <div class="min-w-0">
              <p class="text-sm font-medium text-ink truncate">{{ item.product_title || item.raw_sku }}</p>
        ...
        <p v-else class="text-sm text-muted">No in-stock items with expiry data yet.</p>
      </div>
    </div>
      ...
      <div class="card p-6">
        ...
        <div v-if="!loading && batches.length > 0" class="space-y-2">
          <NuxtLink
            ...
            :key="batch.id"
            :to="`/expiry/${batch.id}`"
            class="flex items-center justify-between rounded-lg border border-line px-4 py-3 transition-colors hover:border-line hover:bg-surface-sunken"
          >
            <div class="min-w-0">
              <p class="text-sm font-medium text-ink">{{ batch.batch_code }}</p>
              <p class="text-xs text-muted">
    ...
    <div v-show="activeTab === 'lifo'" class="space-y-4">
      <div class="card p-6">
        <div class="mb-4">
          <h2 class="text-lg font-semibold text-ink">LIFO Planner</h2>
          <p class="mt-1 text-sm text-muted">Items sorted by expiry date — soonest first. Sell, promote, or dispose of expiring stock.</p>
        </div>
        ...
        <div v-if="lifoItems.length > 0" class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody class="text-ink-soft">
              <tr v-for="item in lifoItems" :key="item.id" class="border-b border-line">
                <td class="py-2.5 pr-4 font-medium text-ink">{{ item.product_title || '—' }}</td>
                <td class="py-2.5 pr-4 font-mono text-xs text-muted">{{ item.raw_sku }}</td>
                <td class="py-2.5 pr-4 text-xs text-muted">{{ item.batch_code }}</td>
    ...
    <div v-show="activeTab === 'microsites'" class="space-y-4">
      <div class="card p-6">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h2 class="text-lg font-semibold text-ink">Expiry Microsites</h2>
            <p class="mt-1 text-sm text-muted">Public pages showing expiry data transparently for your customers.</p>
          </div>
        ...
        <!-- Create form -->
        <div v-if="showMicrositeForm" class="mb-6 rounded-lg border border-line bg-surface-sunken p-4 space-y-3">
          <div class="grid grid-cols-2 gap-4">
            <div>
        ...
        <div v-if="microsites.length > 0" class="space-y-2">
          ...
          >
            <div class="min-w-0">
              ...
              <p class="text-xs text-muted">
                <code class="text-brown">/m/{{ site.slug }}</code>
                <span :class="['ml-2 rounded-full px-2 py-0.5 text-[10px]', site.is_active ? 'bg-success-soft text-success' : 'bg-line text-muted']">
                  {{ site.is_active ? 'Active' : 'Inactive' }}
                </span>
              </p>
            </div>
            <div class="flex items-center gap-3">
              <NuxtLink :to="`/m/${site.slug}`" target="_blank" class="text-xs text-brown hover:text-brown">Preview</NuxtLink>
              <button class="text-xs text-danger hover:text-danger" @click="handleDeleteMicrosite(site.id)">Delete</button>
            </div>
          </div>
        </div>
