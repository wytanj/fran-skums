  ...
  <div>
    <div class="mb-6 flex items-center justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-muted">Organize products into categories</p>
      </div>
    ...
    <Teleport to="body">
      ...
      >
        <div v-if="showForm" class="fixed inset-0 z-50 flex items-center justify-center bg-brown/40 p-4">
          <div class="card w-full max-w-md p-6" @click.stop>
            <h2 class="mb-4 text-lg font-semibold text-ink">
              {{ editingCategory ? 'Edit Category' : 'New Category' }}
            </h2>
            <form class="space-y-4" @submit.prevent="handleSave">
              <div v-if="errorMsg" class="rounded-lg bg-danger-soft px-4 py-3 text-sm text-danger">{{ errorMsg }}</div>
              <div>
    ...
    <div v-if="loading" class="space-y-3">
      <div v-for="i in 4" :key="i" class="card animate-pulse p-4">
        <div class="h-5 w-40 rounded bg-surface-sunken" />
      </div>
    </div>
    ...
    <div v-else class="space-y-2">
      <div v-for="cat in topLevel" :key="cat.id">
        <div class="card flex items-center justify-between p-4 transition-all hover:border-line">
          ...
          <div class="flex items-center gap-1">
            ...
            <button class="btn-ghost !p-1.5 text-danger" @click="handleDelete(cat.id)">
              <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        <div v-for="child in getChildren(cat.id)" :key="child.id" class="ml-8 mt-2">
          <div class="card flex items-center justify-between p-3 transition-all hover:border-line">
            <div class="flex items-center gap-3">
              <svg class="h-4 w-4 text-muted" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 19.5 15-15m0 0H8.25m11.25 0v11.25" />
              </svg>
              <span class="text-sm font-medium text-ink-soft">{{ child.name }}</span>
            </div>
            <div class="flex items-center gap-1">
              ...
              <button class="btn-ghost !p-1 text-danger" @click="handleDelete(child.id)">
                <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                </svg>
              </button>
