  ...
  <div>
    <div class="mb-6 flex items-center justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-muted">Organize products by brand</p>
      </div>
    ...
    <div v-if="loading" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <div v-for="i in 6" :key="i" class="card animate-pulse p-5">
        <div class="h-5 w-32 rounded bg-surface-sunken" />
        <div class="mt-2 h-4 w-48 rounded bg-surface-sunken" />
      </div>
    </div>
    ...
    <div v-else class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <div v-for="brand in brands" :key="brand.id" class="card p-5 transition-all hover:border-line">
        <div class="flex items-start justify-between">
          <div class="flex items-center gap-3">
            <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-yellow-soft text-sm font-bold text-brown">
              {{ brand.name.charAt(0).toUpperCase() }}
            </div>
            <div>
              <h3 class="font-semibold text-ink">{{ brand.name }}</h3>
              <a v-if="brand.website" :href="brand.website" target="_blank" class="text-xs text-brown hover:underline">
                {{ brand.website }}
              </a>
            </div>
          </div>
          <div class="flex items-center gap-1">
            ...
            <button class="btn-ghost !p-1.5 text-danger hover:text-danger" @click="handleDelete(brand.id)">
              <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
