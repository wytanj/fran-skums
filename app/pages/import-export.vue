...
<template>
  <div class="mx-auto max-w-4xl">
    <div class="mb-6">
      <h1 class="text-2xl font-bold text-ink">Import / Export</h1>
      <p class="mt-1 text-sm text-muted">Bulk manage your product data</p>
    </div>
    ...
    <div class="mb-6 card p-4">
      <div class="flex items-center gap-4">
        <div class="flex-1">
          <label class="text-xs font-medium text-muted">Target Schema</label>
          <select
        ...
        <div class="pt-5">
          <span class="inline-flex items-center rounded-full bg-yellow-deep/10 px-2.5 py-0.5 text-xs font-medium text-brown">
    ...
    <div v-if="activeTab === 'import'">
      ...
      <div v-if="step !== 'upload'" class="mb-6 flex items-center gap-2">
        <button
          ...
          :class="[
            ...
            step === s.key || (step === 'importing' && s.key === 'done')
              ? 'bg-yellow-deep/10 text-brown ring-1 ring-yellow-deep/30'
        ...
        <div class="flex-1" />
        <button class="btn-ghost text-xs text-muted" @click="resetImport">
          Start over
        </button>
      </div>
      ...
      <div v-if="step === 'upload'" class="space-y-6">
        <div class="card p-6">
          ...
          <button class="btn-ghost mb-4 text-brown text-xs" @click="downloadTemplate">
          ...
          <div
            :class="[
              ...
              dragOver ? 'border-indigo-500 bg-yellow-soft/50' : 'border-line hover:border-line-strong',
            ]"
          ...
          >
            ...
            <svg class="mb-4 h-12 w-12 text-muted" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5m-13.5-9L12 3m0 0 4.5 4.5M12 3v13.5" />
            </svg>

            <p class="text-sm font-medium text-ink">Drop your product file here, or click to browse</p>
            <p class="mt-1 text-xs text-muted">CSV, TSV, XLS, and XLSX files are supported.</p>
          </div>
        </div>
      </div>
      ...
      <div v-if="step === 'map'" class="space-y-4">
        <div class="card p-6">
          <div class="flex items-center justify-between mb-4">
            <div>
              <h2 class="text-lg font-semibold text-ink">Map Your Columns</h2>
              <p class="mt-0.5 text-sm text-muted">
                We found <span class="text-ink font-medium">{{ csvHeaders.length }}</span> columns
                in <span class="text-ink font-medium">{{ fileName }}</span>
                ({{ csvRows.length.toLocaleString() }} rows
                <span v-if="providerHint === 'abw'" class="text-warning">· ABW catalog detected</span>).
                Auto-matched <span class="text-success font-medium">{{ mappedCount }}</span> columns.
                <span v-if="isLargeImport" class="block mt-1 text-brown">
          ...
          <div v-if="!hasTitleMapped" class="mb-4 rounded-lg bg-warning-soft border border-amber-500/20 px-4 py-3 text-sm text-warning">
            ...
            <table class="w-full">
              <thead>
                <tr class="border-b border-line bg-surface-sunken/80">
                  <th class="px-4 py-3 text-left text-xs font-medium text-muted">Your CSV Column</th>
                  <th class="px-4 py-3 text-left text-xs font-medium text-muted">Sample Data</th>
                  <th class="px-4 py-3 text-left text-xs font-medium text-muted">Maps To</th>
                  <th class="px-2 py-3 text-center text-xs font-medium text-muted w-16">Match</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-line-soft">
                ...
                >
                  <td class="px-4 py-3">
                    <code class="rounded bg-surface-sunken px-2 py-0.5 text-sm text-ink">{{ header }}</code>
                  </td>
                  <td class="px-4 py-3 text-sm text-muted max-w-[200px] truncate">
                    {{ csvRows[0]?.[header] || '—' }}
                  </td>
                  <td class="px-4 py-3">
                    <select
                  ...
                  <td class="px-2 py-3 text-center">
                    <span v-if="getMatchConfidence(header) === 'high'" class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-success-soft text-success" title="High confidence match">
                      <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" />
                      </svg>
                    </span>
                    <span v-else-if="getMatchConfidence(header) === 'low'" class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-warning-soft text-warning" title="Partial match — verify">
                      <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
                      </svg>
                    </span>
                    <span v-else class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-surface-sunken text-muted">
                      <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
        ...
        <div class="card p-6">
          ...
          <div class="overflow-x-auto rounded-lg border border-line">
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-line bg-surface-sunken/80">
                  <th class="px-3 py-2 text-left text-xs font-medium text-muted">#</th>
                  <th
              ...
              <tbody class="divide-y divide-line-soft">
                <tr v-for="(row, idx) in previewRows" :key="idx" class="hover:bg-surface-sunken/20">
                  <td class="px-3 py-2 text-xs text-muted">{{ idx + 1 }}</td>
                  <td
                    v-for="[csvH] in Object.entries(columnMappings).filter(([, v]) => v)"
                    :key="csvH"
                    class="px-3 py-2 text-ink-soft max-w-[180px] truncate"
                  >
                    {{ row[csvH] || '' }}
                  </td>
                </tr>
          ...
          <div class="mt-4 flex items-center justify-between">
            <div class="text-sm text-muted space-y-1">
              <p>
                Ready to import <span class="text-ink font-medium">{{ csvRows.length.toLocaleString() }}</span> products with
                <span class="text-ink font-medium">{{ mappedCount }}</span> fields each.
              </p>
      ...
      <div v-if="step === 'importing'" class="card p-6">
        <div class="flex items-start gap-4">
          <svg class="mt-1 h-8 w-8 shrink-0 animate-spin text-brown" fill="none" viewBox="0 0 24 24">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-start justify-between gap-4">
              <div class="min-w-0">
                <h3 class="text-lg font-semibold text-ink">{{ importProgress.phase }}</h3>
                <p class="mt-1 truncate text-sm text-muted">{{ importProgress.detail || 'Starting import' }}</p>
                <p v-if="activeJobId" class="mt-2 text-xs font-mono text-muted">
                  Job {{ activeJobId }}
                  <span v-if="jobSnapshot?.status" class="ml-2 rounded bg-surface-sunken px-1.5 py-0.5 text-brown">
                    {{ jobSnapshot.status }}
                  </span>
                </p>
              </div>
              <div class="shrink-0 text-right">
                <p class="text-2xl font-bold text-ink">{{ importProgressPercent }}%</p>
                <p class="text-xs text-muted">
            ...
            <div class="mt-5 h-2 overflow-hidden rounded-full bg-surface-sunken">
              <div
                class="h-full rounded-full bg-yellow-deep transition-all duration-300"
            ...
            <div class="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
              <div class="rounded-lg bg-white/70 p-3">
                ...
                <p class="mt-1 text-lg font-semibold text-ink">{{ importProgress.current.toLocaleString() }}</p>
              </div>
              <div class="rounded-lg bg-success-soft p-3">
                <p class="text-xs text-success/70">Committed</p>
                <p class="mt-1 text-lg font-semibold text-success">{{ importProgress.success.toLocaleString() }}</p>
              </div>
              <div class="rounded-lg bg-blue-soft p-3">
                <p class="text-xs text-brown/70">Created / Updated</p>
                <p class="mt-1 text-lg font-semibold text-brown">
                  {{ importProgress.created.toLocaleString() }} / {{ importProgress.updated.toLocaleString() }}
                </p>
              </div>
              <div class="rounded-lg bg-danger-soft p-3">
                <p class="text-xs text-danger/70">Errors</p>
                <p class="mt-1 text-lg font-semibold text-danger">{{ importProgress.errors.toLocaleString() }}</p>
              </div>
            </div>
        ...
        <div class="card p-6">
          <div class="mb-4 flex items-start justify-between gap-4">
            <div>
              ...
              <p v-if="importResult.jobId" class="mt-1 text-xs font-mono text-muted">
                Job {{ importResult.jobId }}
                <span v-if="jobSnapshot?.status" class="ml-2 text-success">· {{ jobSnapshot.status }}</span>
              </p>
            </div>
            <span
              v-if="jobSnapshot?.status === 'completed' || importResult.success"
              class="rounded-full bg-emerald-500/15 px-3 py-1 text-xs font-medium text-success"
            >
              Completed
            </span>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4 mb-4 sm:grid-cols-4">
            <div class="rounded-lg bg-success-soft p-5 text-center">
              <p class="text-3xl font-bold text-success">{{ importResult.success.toLocaleString() }}</p>
              <p class="mt-1 text-sm text-success/70">Committed</p>
            </div>
            <div class="rounded-lg bg-blue-soft p-5 text-center">
              <p class="text-3xl font-bold text-brown">{{ (importResult.created || 0).toLocaleString() }}</p>
              <p class="mt-1 text-sm text-brown/70">Created</p>
            </div>
            <div class="rounded-lg bg-yellow-deep/10 p-5 text-center">
              <p class="text-3xl font-bold text-brown">{{ (importResult.updated || 0).toLocaleString() }}</p>
              <p class="mt-1 text-sm text-brown/70">Updated</p>
            </div>
            <div class="rounded-lg bg-danger-soft p-5 text-center">
              <p class="text-3xl font-bold text-danger">{{ importResult.errors.toLocaleString() }}</p>
              <p class="mt-1 text-sm text-danger/70">Errors</p>
            </div>
          </div>

          <div v-if="importResult.messages.length > 0" class="mb-4">
            <p class="mb-2 text-xs font-medium text-muted">Error details (first {{ importResult.messages.length }}):</p>
            <div class="max-h-48 overflow-y-auto rounded-lg bg-surface-sunken p-3 space-y-1">
              <p v-for="(msg, i) in importResult.messages" :key="i" class="text-xs text-muted font-mono">
                {{ msg }}
              </p>
            </div>
          </div>
    ...
    <div v-if="activeTab === 'export'" class="card p-6">
      <h2 class="mb-2 text-lg font-semibold text-ink">Export Products</h2>
      <p class="mb-4 text-sm text-muted">
      ...
      <div class="mb-6 space-y-4">
        <template v-for="group in fieldGroups" :key="group">
          <div>
            <p class="mb-2 text-xs font-medium text-muted uppercase tracking-wider">{{ group }}</p>
            <div class="grid grid-cols-2 gap-2 sm:grid-cols-3">
              <label
                ...
                :class="[
                  ...
                  field.selected ? 'bg-yellow-soft text-brown ring-1 ring-yellow-deep/20' : 'bg-surface-sunken text-muted hover:bg-line',
                ]"
              >
                <input
