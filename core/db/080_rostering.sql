-- ============================================================
-- SKUMS — Store rostering (employees, zones, hourly shifts)
--
-- Purpose:
--   HQ plans who works which floor zone by the hour.
--   Employees can be manual or imported from Rippling.
--   POS reads "my current zone" for logged-in staff.
--
-- Run AFTER: 079_marketplace_read_path_correctness.sql
-- ============================================================

-- ── Zones (Zone 1/2/3, Cashier, Back of House) ───────────────
...
create table if not exists public.roster_employees (
  ...
  external_id           text,
  -- Link to fran-pos pos_staff_members.id (or other POS staff ref)
  pos_staff_ref         text,
...
create table if not exists public.roster_shifts (
  ...
  -- Hour-resolution intended; stored as timestamptz for TZ honesty
