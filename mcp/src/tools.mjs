...
export const toolDefinitions = [
  ...
  // ── Roster (hourly + zone) ──
  {
    name: 'roster_list_zones',
    description:
      'List store floor zones (Zone 1/2/3, Cashier, Back of House). Seeds defaults if empty. roster:read / safe.',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'roster_list_employees',
    description:
      'List roster employees (manual or Rippling-imported). roster:read / safe.',
    inputSchema: {
      type: 'object',
      properties: {
        q: { type: 'string' },
        source_provider: { type: 'string', enum: ['manual', 'rippling', 'import', 'other'] },
  ...
  {
    name: 'roster_upsert_employee',
    description:
      'Create/update a roster employee manually. Set source_provider=manual (default) or link pos_staff_ref for POS. roster:write.',
    inputSchema: {
      type: 'object',
      properties: {
        id: { type: 'string' },
        ...
        pos_staff_ref: { type: 'string', description: 'fran-pos staff member id' },
  ...
  {
    name: 'roster_import_rippling',
    description:
      'Import employees from Rippling-shaped JSON workers (no live Rippling API). Upserts by external_id. roster:write.',
    inputSchema: {
      type: 'object',
      properties: {
        workers: {
          type: 'array',
          items: { type: 'object' },
          description: '[{id, name|display_name, email, phone, role, title, department, ...}]',
        },
      },
      required: ['workers'],
    },
  },
  {
    ...
    description:
      'List hourly shifts (optional from/to ISO, employee_id, zone_id). roster:read / safe.',
    inputSchema: {
      type: 'object',
      properties: {
        from: { type: 'string' },
  ...
  {
    name: 'roster_upsert_shift',
    description:
      'Create/update an hourly shift for an employee in a zone. starts_at/ends_at floored to the hour. roster:write.',
    inputSchema: {
      type: 'object',
      properties: {
        id: { type: 'string' },
        employee_id: { type: 'string' },
        zone_id: { type: 'string' },
        starts_at: { type: 'string', description: 'ISO datetime (hour)' },
        ends_at: { type: 'string' },
        hours: { type: 'number', description: 'Alt to ends_at: duration hours' },
        status: { type: 'string' },
        notes: { type: 'string' },
        pos_location_id: { type: 'string' },
      },
      required: ['employee_id', 'zone_id', 'starts_at'],
    },
  },
  {
    name: 'roster_cancel_shift',
    description: 'Cancel a shift (status=cancelled). roster:write.',
    inputSchema: {
      type: 'object',
  ...
  },
  {
    name: 'roster_board',
    description:
      'Day board: shifts by zone for a date (Asia/Singapore default). Prefer for “who is on floor today”. roster:read / safe.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'roster_my_assignment',
    description:
      'Current zone for a POS staff member (pos_staff_ref) or employee_id at a point in time. POS/header zone badge. roster:read or pos:read / safe.',
    inputSchema: {
      type: 'object',
      properties: {
        pos_staff_ref: { type: 'string', description: 'fran-pos staff member id' },
        ...
        employee_id: { type: 'string' },
        external_id: { type: 'string' },
        at: { type: 'string', description: 'ISO now default' },
      },
    },
  },
  ...
  },
  {
    name: 'reports_list',
    description:
      'Rpt-4 List agentic report packs (subscription toggle, schedule, last run). Includes daily-stockout. reports:read. Prefer over inventing digests.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        template_slug: {
          type: 'string',
          description: 'e.g. daily-stockout | marketing-weekly | warehouse-weekly-baseline',
        },
        run_id: { type: 'string' },
      },
    },
  },
  {
    name: 'reports_run',
    description:
      'Rpt-4 Run an enabled report pack now (live Rpt-6 sections). Suggest-only — never approve/Loft/FOB. ' +
      'Daily stockout: template_slug=daily-stockout (per-store ATS=0). Pack must be enabled, or force=true with reports:write. reports:run.',
    inputSchema: {
      type: 'object',
      properties: {
        subscription_id: { type: 'string' },
        template_slug: {
          type: 'string',
          description:
            'e.g. daily-stockout | marketing-weekly | warehouse-weekly-baseline | finance-stock-rewards',
        },
...
export async function handleTool(name, args = {}) {
  ...
  try {
    ...
    switch (name) {
      ...
      // ── Roster ──
      case 'roster_list_zones': {
        requireScope('roster:read')
        const zones = await roster.listZones(requireWorkspaceId(), {
          active_only: a.active_only !== false,
        })
        return jsonResult({ zones, deep_link: '/roster' })
      }
      case 'roster_list_employees': {
        requireScope('roster:read')
        const employees = await roster.listEmployees(requireWorkspaceId(), {
          q: a.q,
          source_provider: a.source_provider,
          employment_status: a.employment_status,
          limit: a.limit,
        })
        return jsonResult({ employees, deep_link: '/roster' })
      }
      case 'roster_upsert_employee': {
        requireScope('roster:write')
        const employee = await roster.upsertEmployee(requireWorkspaceId(), a)
        return withMcpAudit(
          name,
          requestId,
          {
            object_type: 'roster_employees',
            entity_id: employee.id,
            status: employee.employment_status,
            operation: a.id ? 'UPDATE' : 'INSERT',
            after_data: employee,
          },
          { employee, deep_link: '/roster' },
        )
      }
      case 'roster_import_rippling': {
        requireScope('roster:write')
        const result = await roster.importRipplingEmployees(
          requireWorkspaceId(),
          Array.isArray(a.workers) ? a.workers : [],
        )
        return withMcpAudit(
          name,
          requestId,
          {
            object_type: 'roster_employees',
            entity_id: result.employees?.[0]?.id || requireWorkspaceId(),
            status: 'imported',
            operation: 'INSERT',
            after_data: result,
          },
          { ...result, deep_link: '/roster', note: 'Rippling-shaped import only — no live Rippling API call.' },
        )
      }
      case 'roster_list_shifts': {
        requireScope('roster:read')
        const shifts = await roster.listShifts(requireWorkspaceId(), {
          from: a.from,
          to: a.to,
        ...
        return jsonResult({ shifts, deep_link: '/roster' })
      }
      case 'roster_upsert_shift': {
        requireScope('roster:write')
        const shift = await roster.upsertShift(requireWorkspaceId(), a)
        return withMcpAudit(
          name,
          requestId,
          {
            object_type: 'roster_shifts',
            entity_id: shift.id,
            status: shift.status,
            operation: a.id ? 'UPDATE' : 'INSERT',
            after_data: shift,
          },
          { shift, deep_link: '/roster' },
        )
      }
      case 'roster_cancel_shift': {
        requireScope('roster:write')
        const shift = await roster.cancelShift(requireWorkspaceId(), String(a.shift_id))
        return withMcpAudit(
          name,
          requestId,
          {
      ...
      case 'roster_board': {
        requireScope('roster:read')
        const board = await roster.getBoard(requireWorkspaceId(), {
          date: a.date,
          timezone: a.timezone || 'Asia/Singapore',
        })
        return jsonResult({ board, deep_link: '/roster' })
      }
      case 'roster_my_assignment': {
        requireScope('roster:read')
        const assignment = await roster.getMyAssignment(requireWorkspaceId(), {
          pos_staff_ref: a.pos_staff_ref,
          staff_ref: a.staff_ref,
          employee_id: a.employee_id,
          external_id: a.external_id,
          at: a.at,
        })
        return jsonResult({ assignment, deep_link: '/roster' })
      }
